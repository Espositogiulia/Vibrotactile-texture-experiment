...
for i=1:4;
    ...
    lwdata(i).ttest.grand_avg_mean_base_chan_avg_bl=mean(lwdata(i).ttest.mean_base_chan_avg_bl); % added 24/05/24 
    lwdata(i).ttest.stdev_mean_base_chan_avg_bl=std(lwdata(i).ttest.mean_base_chan_avg_bl); % added 24/05/24 
...
%     [a,b]=ttest(mean_tp_oddball);
    ...
    lwdata(i).ttest.grand_avg_mean_oddball_chan_avg_bl=mean(lwdata(i).ttest.mean_oddball_chan_avg_bl); % added 24/05/24
    lwdata(i).ttest.stdev_mean_oddball_chan_avg_bl=std(lwdata(i).ttest.mean_oddball_chan_avg_bl); % added 24/05/24 
...
% to position each topoplot
positionsTopoOddball = zeros(4,4); 
...
%% plot channels to show difference between hand and foot
subplot(1,2,1);
plot([1:5], lwdata(3).mean_base_signal([ 45 15 16 46 17]),'-o','MarkerFaceColor', [24, 116, 205]/255, ...
    'Color', [24, 116, 205]/255,'LineWidth',2); % c1 c3 cz c2 c4
set(gca,'XTick',[1 2 3 4 5]);
set(gca,'XTickLabel',{'C1', 'C3', 'Cz', 'C2', 'C4'},'FontSize',14);
xlim([0 6]);
hold on;
plot([1:5], lwdata(4).mean_base_signal([45 15 16 46 17]),'-o', 'MarkerFaceColor', [238, 44, 44]/255,...
    'Color', [238, 44, 44]/255,'LineWidth',2);
title('Base response','FontSize',16,'FontWeight','bold');
legend('Hand', 'Foot');
hold off;
subplot(1,2,2);
plot([1:5], lwdata(3).mean_oddball_signal([45 15 16 46 17]),'-o','MarkerFaceColor', [24, 116, 205]/255, ...
    'Color', [24, 116, 205]/255,'LineWidth',2);
set(gca,'XTick',[1 2 3 4 5]);
set(gca,'XTickLabel',{'C1', 'C3', 'Cz', 'C2', 'C4'},'FontSize',14);
xlim([0 6]);
hold on;
plot([1:5], lwdata(4).mean_oddball_signal([45 15 16 46 17]),'-o','MarkerFaceColor', [238, 44, 44]/255,...
    'Color',[238, 44, 44]/255,'LineWidth',2);
title('Oddball response','FontSize',16,'FontWeight','bold');
legend('Hand', 'Foot');

%% save workspace
filename = 'analysis_group_level_2705.mat';
save (filename, '-v7.3');

%% save .csv files for topographical RM anova test in JASP
...
writetable(base_data_anova_excl_wn2,'base_data_anova_wn2_excl_2410.csv')
writetable(oddball_data_anova_excl_wn2,'oddball_data_anova_wn2_excl_2410.csv')