...
%% present experiment for n trials (order depends on condCountTot row ie ppt ID)
for thisTrial = 1:size(randTrialsHand,2) % 16 trials
        for thisPpt = 1:size(randTrialsHand,1) %15 ppt
            if ppt ==thisPpt
                data = trials{randTrialsHand(thisPpt,thisTrial)};
                queueOutputData (NI.NI,data);
                prepare(NI.NI);   %prepare the NI
                display(['queueing data ' num2str(randTrialsHand(thisPpt,thisTrial))]); %present white noise according to participant and trial number
                pause(5);
                ...
                deviantsHand(thisTrial) = input(prompt);
            end
        end
end
    
save(['deviantsHand' num2str(ppt) '.mat'], 'deviantsHand');
